...
def evaluate_regression_model(model_list:list,x_train:np.ndarray,y_train:np.ndarray,x_test:np.ndarray,y_test:np.ndarray,base_accuracy:float=0.6)->MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            train_acc = r2_score(y_train_pred,y_train)
            test_acc = r2_score(y_test_pred,y_test) 
...
class ModelFactory:
    ...
    def class_for_name(module_name:str,class_name:str):
        try:
            #Load the module,raise the importerror if module cannot found
            module = importlib.import_module(module_name)

            #Get the class,raise the attributeerror if class not found
    ...
    def get_initialized_model_list(self)->List[InitializedModelDetail]:
        ...
        try:
            ...
            for model_serial_number in self.models_initialization_config.keys():
                ...
                model_obj_ref = ModelFactory.class_for_name(model_initialization_config[MODULE_KEY],model_initialization_config[CLASS_KEY])
                model = model_obj_ref()
